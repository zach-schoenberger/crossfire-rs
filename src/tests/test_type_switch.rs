use crate::tests::common::{async_spawn, runtime_block_on, sleep, timeout};
use crate::*;
...
fn test_bounded_async_with_sync_receiver_switch_buffered<T: AsyncTxTrait<usize>>(
    ...
    runtime_block_on_with_timeout!(async move {
        ...
        let (rx, async_received) = receiver_task.await.expect("Async receiver task failed");
        ...
        let sync_received = std::thread::spawn(move || {
        ...
        .join()
        .expect("Sync receiver thread panicked");
...
fn test_spsc_bounded_blocking_with_async_receiver_switch(
    ...
    runtime_block_on_with_timeout!(async move {
        ...
        let async_received = async_receiver_task.await.expect("Async receiver task failed");
...
fn test_spsc_bounded_async_with_blocking_sender_switch(
    ...
    runtime_block_on_with_timeout!(async move {
        ...
        let tx = sender_task.await.expect("Sender task failed");
        ...
        let all_received = receiver_task.await.expect("Receiver task failed");
